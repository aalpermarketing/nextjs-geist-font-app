<?php
// Vitrin verilerini çekmek için örnek kod
...
function fetchFromAPI($url, $api_key, $kategori = 'all') {
    ...
    // cURL ayarları
    curl_setopt($ch, CURLOPT_URL, $url . '?api_key=' . $api_key . '&kategori=' . $kategori);
    curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false); // SSL sertifika doğrulamasını atla (geliştirme için)
    ...
    // JSON yanıtı diziye çevir
    return json_decode($response, true);
}

// Tüm vitrinleri çek
$data = fetchFromAPI($api_url, $api_key);

if ($data && $data['success']) {
    // Vitrinleri kategorilerine göre ayır
    $vitrinler = $data['data']['vitrinler'];
    $popuplar = $data['data']['popuplar'];
    
    $vip_vitrinler = array_filter($vitrinler, function($v) {
        return $v['kategori'] === 'vip';
    });
    
    $gold_vitrinler = array_filter($vitrinler, function($v) {
        return $v['kategori'] === 'gold';
    });
    
    $silver_vitrinler = array_filter($vitrinler, function($v) {
        return $v['kategori'] === 'silver';
    });
} else {
    die("API'den veri çekilemedi!");
}
?>
<!DOCTYPE html>
<html lang="tr">
